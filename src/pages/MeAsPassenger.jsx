...
function MeAsPassenger() {
  const [items, setItems] = useState([]);
  const { currentUser } = useSelector((state) => state.user);

  useEffect(() => {
    const fetchPassengerJourneys = async () => {
      try {
        const response = await fetch(`/api/journey/get-passenger?id=${currentUser._id}`);
        const data = await response.json();
        setItems(data);
      } catch (error) {
        console.error('Error fetching journeys as passenger:', error);
      }
    };
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-blue-50 to-indigo-50 min-h-screen">
      <h2 className="text-3xl font-extrabold text-indigo-800 mb-6 border-b-2 border-indigo-300 pb-2 flex items-center gap-2">
        <Car className="text-indigo-600" /> Journeys Where You're the Passenger
      </h2>
